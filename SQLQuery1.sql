...
----Find Top 1 outlet by Cuisine without using top and limit.

with cte as(
select cuisine,restaurant_id, count(*) as no_of_orders from orders
group by cuisine,restaurant_id)
select * from
(select *,ROW_NUMBER() over(partition by cuisine order by no_of_orders desc) as rn from cte) as rn
where rn=1;
...
with cte1 as(
select customer_code,cast(min(placed_at) as date) as first_order from orders
group by Customer_code)
select first_order, count(*) as Tota_Customer_count from cte1
group by first_order;



----COUNT OF ALL THE USERS WHO WERE ACQUIRED IN JANUARY 2025 AND ONLY PLACED ONE ORDER IN JANUARY
----AND DID NOT PLACE ANY OTHER ORDER.

select customer_code,count(*) as cust_count from orders
where month(placed_at)=1 and year(placed_at)=2025 and Customer_code not in (select customer_code from orders
...
where  not(month(placed_at)=1 and year(placed_at)=2025)
--group by customer_code -- this query helps to find who are the people who did order except 2025 January.



----LIST ALL THE CUSTOMERS WITH NO ORDER IN THE LAST 7 DAYS BUT WERE ACQUIRED ONE MONTH AGO WITH THEIR 
...
group by customer_code)
select cte2.*,Promo_code_Name as First_order_promo from cte2
inner join orders on cte2.customer_code=orders.Customer_code and cte2.first_order_date=orders.Placed_at
where latest_order_date< dateadd(day,-7,getdate()) and first_order_date<dateadd(month,-1,getdate()) and Promo_code_Name is not null;




----GROWTH TEAM IS PLANNING TO CREATE A TRIGGER THAT WILL TARGET CUSTOMERS AFTER THEIR EVERY
----THIRD ORDER WITH A PERSONALIZED COMMUNICATION AND THEY HAVE ASKED ME TO A QUERY FOR THIS.

with cte4 as(
select customer_code,placed_at,row_number() over(partition by customer_code order by placed_at) as number_of_orders from orders)
select* from cte4 where number_of_orders%3=0  ---This will give the cutomercode of 3 multiply but it keeps old repetative records with new records
--and cast(placed_at as date)=cast(getdate() as date)---- if this adds , it gives you latest record as of today not old repetative records.
...
select customer_code,count(*) number_of_orders , count(promo_code_name) as Promo from orders
group by customer_code
...
select count(case when rn=1 and promo_code_name is not null then Customer_code end )*100.00/count(distinct customer_code) as Percentage_of_Cust from cte5;
